...
// 2. Map the array of inventors into a new array. 
// This new array should only contain objects with the inventors' first and last names.

// - For each inventor, return an object in the format { first: "First Name", last: "Last Name" }.
// - The new array should be a collection of these objects, each representing an inventor with only 
// their first and last names.

// Hint:  Return a new object literal from the callback that looks something 
// like { first: "First Name", last: "Last Name" }
...
let inventorNamedAda = inventors.find((name)=>{
...
// Enter your solution code here:
let firstLast = inventors.map((inventors)=>{
    return inventors.first + ' ' + inventors.last;
}).join(', ');

let firstLast = firstLast.split(', ')
